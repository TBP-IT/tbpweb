...
class UserProfile(models.Model):
    ...
    perm_address1 = models.CharField(
        ...
        help_text='Either a permanent or an international address is required.')
    ...
    def get_alumnus_profile(self):
        try:
            return Alumnus.objects.get(user=self.user)
    ...
    def is_candidate(self, current=True):
        ...
        if profile:
            return profile.is_candidate(current) and not profile.is_member()
        else:
            return False
    ...
    def has_alumnus_profile(self):
        """Return True if this person has an alumnus profile, False otherwise.
        """
        return self.get_alumnus_profile() is not None
